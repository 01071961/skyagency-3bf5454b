 ...
 * 
 * IMPORTANT: User must configure the portal in Stripe Dashboard first:
...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      throw new Error("No Stripe customer found for this user. Subscribe to a plan first.");
    }
    ...
    logStep("Customer portal session created", { 
      ...
      url: portalSession.url.slice(0, 50) + "..." 
    });
