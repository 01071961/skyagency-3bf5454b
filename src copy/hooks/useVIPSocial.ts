...
// Hook for live chat with deduplication
export function useVIPLiveChat(postId: string) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const authorIds = [...new Set(chatMessages.map(m => m.author_id))];
      const { data: profiles } = await supabase
        .from('profiles')
        .select('user_id, name, avatar_url')
        .in('user_id', authorIds);
      ...
      const messagesWithAuthors = chatMessages.map(msg => {
      ...
      // Track processed IDs
      processedIds.current = new Set(messagesWithAuthors.map(m => m.id));
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (processedIds.current.has(newMsg.id)) {
            console.log('[VIPLiveChat] Duplicate blocked:', newMsg.id);
            return;
          }
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      // Pre-add to processed IDs to prevent duplicate from realtime
      if (data) {
